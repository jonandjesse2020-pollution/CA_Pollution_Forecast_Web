...
def load_and_merge_data():
    """Loads both AI and CAMS datasets, renames columns, and merges them."""
    # Load AI Data
    ai_df = pd.read_csv("v19_pollution_forecast.csv")
    ai_df['time'] = pd.to_datetime(ai_df['time'])
    
    # Load CAMS Data
    cams_df = pd.read_csv("cams_pollution_forecast.csv")
    ...
    # Rename CAMS columns
    cams_rename_map = {
        'PM25_concentration': 'CAMS_PM25', 'PM10_concentration': 'CAMS_PM10',
        'O3_concentration': 'CAMS_O3', 'NO2_concentration': 'CAMS_NO2',
        'SO2_concentration': 'CAMS_SO2', 'CO_concentration': 'CAMS_CO'
    }
    cams_df = cams_df.rename(columns=cams_rename_map)

    # Merge the two datasets on time, lat, and lon
    merged_df = pd.merge(ai_df, cams_df, on=['time', 'lat', 'lon'], how='inner')
    return merged_df
...
def get_forecast_data(user_lat, user_lon, _df):
    """
    Finds the nearest coordinates in the merged dataset to the user's location
...
# ==========================================
# Load the master merged dataset once on startup
    ...
    if lat and lon:
        ...
        for i, (tab_name, meta) in enumerate(POLLUTANTS.items()):
            with tabs[i]:
                ...
                # Plot CAMS Baseline
                if f"CAMS_{pol_key}" in df.columns:
                    fig.add_trace(go.Scatter(
                        x=df["Target_Time_UTC"], y=df[f"CAMS_{pol_key}"],
                        mode='lines+markers',
                        name='CAMS Baseline',
                        line=dict(color='#1f77b4', width=2, dash='dash'), # Blue dashed line
                        marker=dict(size=6)
                    ))
        ...
        # CAMS Table
        with st.expander("📊 View Raw Data (CAMS Baseline)"):
            cams_cols = ["Target_Time_UTC"] + [col for col in df.columns if col.startswith("CAMS_")]
            st.dataframe(format_table(df[cams_cols]), use_container_width=True)
